...
def default_conf():
    ...
    # @todo wavemix is 0 for KD
    basedataset = DynamicIngredient("audioset.dataset.dataset", wavmix=0, add_index=True)
    ...
    preds_file = "passt_enemble_logits_mAP_495.npy"
    preds = os.path.join(preds_folder, preds_file)
    temperature = 1
    kd_lambda = 0.9
    dist_activation = "sigmoid"
...
class M(Ba3lModule):
    def __init__(self, experiment):
        ...
        self.student_temperature = self.config.get("student_temperature", self.temperature)
        self.kd_lambda = self.config.get("kd_lambda", 0.5)  # weight loss for soft targets
        dist_activation = self.config.get("dist_activation", "softmax")
        if dist_activation == "softmax":
            self.dist_activation = torch.nn.LogSoftmax(dim=-1)
            tgt_activation = partial(F.log_softmax, dim=-1)
            self.dist_loss = torch.nn.KLDivLoss(log_target=True, reduction="none")  # KL Divergence loss for soft targets
        elif dist_activation == "sigmoid":
            self.dist_activation = torch.nn.Sequential()
            tgt_activation = torch.sigmoid
            self.dist_loss = torch.nn.BCEWithLogitsLoss(reduction="none")  # KL Divergence loss for soft targets
        else:
            raise ValueError(f"Unknown distillation activation: {dist_activation}")

        # load stored predictions
        print(self.config['preds'])
        with open(self.config['preds'], 'rb') as f:
            self.predictions = np.load(f)

        # apply temperature and activation function to predictions
        self.predictions = torch.from_numpy(self.predictions).float()
        self.predictions = tgt_activation(self.predictions / self.temperature)
        self.predictions.requires_grad = False
    ...
    def training_step(self, batch, batch_idx):
        ...
        # distillation loss
        if self.kd_lambda > 0:
            y_soft_teacher = self.predictions[i.cpu()]
            y_soft_teacher = y_soft_teacher.to(y_hat.device).type_as(y_hat)

            # Temperature adjusted probabilities of teacher and student
            y_soft_student = self.dist_activation(y_hat / self.student_temperature)

            if self.mixup_alpha:
                soft_targets_loss = self.dist_loss(y_soft_student, y_soft_teacher).mean(dim=1) * lam.reshape(
                    batch_size) + \
                                    self.dist_loss(y_soft_student, y_soft_teacher[rn_indices]).mean(dim=1) * (
                                                1. - lam.reshape(batch_size))
                soft_targets_loss = soft_targets_loss.mean()
            else:
                soft_targets_loss = self.dist_loss(y_soft_student, y_soft_teacher).mean()  # mean since reduction="none"

            # weighting losses
            soft_targets_loss = self.kd_lambda * soft_targets_loss
            label_loss = (1 - self.kd_lambda) * label_loss
            loss = label_loss + soft_targets_loss
        else:
            loss = label_loss
            soft_targets_loss = torch.tensor(0., device=label_loss.device, dtype=label_loss.dtype)

        results = {"loss": loss, "teacher_loss": soft_targets_loss.detach(),
    ...
    def training_epoch_end(self, outputs):
        ...
        avg_teacher_loss = torch.stack([x['teacher_loss'] for x in outputs]).mean()
        logs['label_loss'] = avg_label_loss
        logs['teacher_loss'] = avg_teacher_loss
